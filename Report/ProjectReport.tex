...
\begin{figure}[H]
\includegraphics[scale=.5]{img/solution.png}
\centering
...
The Bartels-Stewart algorithm \cite{Bartels} \cite{Simoncini} can be used to solve the Sylvester equation $AX + XB = F$. In the general case the algorithm is as follows:
\begin{enumerate}
...
In this case, $A=B=T$ and $X=U$, so $P=Q$ and $P^{-1}TP = \text{diag}(\lambda_1, \dots, \lambda_n)$, where $\lambda_1, \dots, \lambda_n$ are the eigenvalues of $T$. Letting $\tilde{F}=P^{-1}FP$, the solution is:
\[ U = P \tilde{U} P^{-1}, \text{ with } \tilde{u}_{ij} = \frac{\tilde{f}_{ij}}{\lambda_i + \lambda_j} \]

\subsubsection*{Using numpy.linalg.eig}
The eigenvalues and eigenvectors can be computed using NumPy's \texttt{linalg.eig} function. The results using this are given in Figure 7.
...
Here the experimental order of convergence moves away from 2 as $n$ is increased. This is likely due to the fact that as there is no general formula for calculating eigenvalues and eigenvectors of any matrix, the eigenvalues and eigenvectors are approximated by NumPy's \texttt{linalg.eig}.
...
\subsubsection{Modified Conjugate Gradient Algorithm}
In \cite{Hou} a modified conjugate gradient (MCG) algorithm is proposed, which is adapted for Sylvester Equations. The algorithm is as follows:
\begin{enumerate}
