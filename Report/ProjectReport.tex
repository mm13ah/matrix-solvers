...
Here the experimental order of convergence moves away from 2 as $n$ is increased. This is likely due to the fact that as there is no general formula for calculating eigenvalues and eigenvectors for an arbitrary matrix, the eigenvalues and eigenvectors are most likely approximated by NumPy's \texttt{linalg.eig}.
...
\section{Uncertainty}
By introducing a random diffusion coefficient $\varepsilon$ into (1.1), the PDE to be solved now becomes:
\begin{equation}
