...
\begin{itemize}
\item To carry out an extensive literature review on methods for solving matrix equations (both direct and iterative) from a wide range of sources. To decide which of these methods are appropriate to implement and to gain a solid understanding of how they work. 
...
\item To study methods for solving random PDEs and adapt the methods implemented previously so they can be used to solve PDEs of this type.
...
\end{equation}
where $I$ is the identity matrix of dimension $(P+1)$. We can then solve for $C$ and reshape it back into a vector, which vastly speeds up computation time and drastically reduces memory requirements. The eigenvalues and eigenvectors of $A$ are simply 2 and the columns of the identity matrix, respectively. As $B$ is a square matrix with dimension $(P+1)$, as long as $K$ is chosen not to be too large, computing the eigenpairs of $B$ will be cheap and approximating them will not have a large impact on the solution. We can therefore use \texttt{numpy.linalg.eig} to calculate the eigenpairs.

Results using the conjugate gradient method \texttt{sparse.linalg.cg}, with a convergence tolerance of $10^{-9}$, to solve for $C$ and the similarity transformation method to solve for $X$ are given in Table \ref{table:stochastic sylvester}. Here we to used \texttt{numpy.linalg.eigs} to compute the eigenpairs of $B$.
...
\end{equation}
where $\Psi$ denote the Legendre polynomials, as in the single parameter case. Next we multiply by $\Psi_k$ and take the inner product, which gives:
\begin{equation}
\sum_{i=0}^P (-(u_i)_{xx} - (u_i)_{yy}) \langle \Psi_k \varepsilon \Psi_i \rangle = \langle f \Psi_k \rangle
\end{equation}
for $k=0,\dots,P$. Here we have:
\begin{equation}
\langle \Psi_i \varepsilon \Psi_k \rangle = \sum_{p,q=1}^N 2^{-(p+q)} \sin(p\pi x)\sin(q\pi y) \langle \Psi_i \varepsilon_{pq} \Psi_k \rangle
\end{equation}
and:
\begin{equation}
\langle f \Psi_k \rangle = 2 \pi^2 \sin(\pi x)\sin(\pi y) \sum_{p,q=1}^N 2^{-(p+q)} \sin(p \pi x) \sin(q \pi y) \langle \Psi_k \varepsilon_{pq} \rangle  \nonumber
\end{equation}
\begin{equation}
+ \left(\pi^2 \sum_{p,q=1}^N 2^{-(p+q)} (p^2 + q^2) \sin(p \pi x)\sin(q \pi y) \langle \Psi_k \varepsilon_{pq} \rangle \right) \left(\sum_{p,q=1}^N 2^{-(p+q)}  \sin(p \pi x)\sin(q \pi y) \langle \Psi_k \varepsilon_{pq} \rangle \right)
\end{equation}
We now have a deterministic coupled set of differential equations which we can solve. As before, we rewrite (4.41) into a form that we can solve. As $\varepsilon_{pq} \sim u(-1,1)$ for all $p,q=1 \dots, N$, varying $p$ and $q$ does not change the inner (as the integral will be evaluated using the same limits and probability density function), therefore we can represent the inner products as:
\begin{equation}
\langle \Psi_i \varepsilon_{pq} \Psi_k \rangle = \langle \Psi_i \varepsilon_0 \Psi_k \rangle
\end{equation}
and 
\begin{equation}
\langle \Psi_k \varepsilon_{pq} \rangle = \langle \Psi_k \varepsilon_0 \rangle
\end{equation}
for all $p,q=1 \dots N$. This means we can remove the inner product from the summations, separating the random and spatial dimensions. We use the finite difference method to discretise the system in space and represent the unknowns $u_i$ as a single vector:
\begin{equation}
Lu_i = -(u_i)_xx - (u_i)_yy \sum_{p,q=1} 2^{-(p+q)} \sin(p \pi x)\sin(q \pi y)
\end{equation}
